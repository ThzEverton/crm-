...
export class OperationsController {
  ...
  createAppointment = (req: Request, res: Response): void => { try { this.service.createAppointment(req.body); res.redirect('/agenda?notice=appointment-created') } catch (e) { if (e instanceof ZodError) { res.redirect('/agenda?error=Revise os dados da consulta.'); return }; if (e instanceof Error) { res.redirect(`/agenda?error=${encodeURIComponent(e.message)}`); return }; throw e } }
  availability = (req: Request, res: Response): void => { res.json({ slots: this.service.availability(String(req.query.date ?? ''), Number(req.query.duration ?? 60)) }) }
  whatsappReminder = (req: Request, res: Response, next: NextFunction): void => { const url = this.service.whatsappReminder(idFrom(req)); if (!url) { next(); return }; res.redirect(url) }
